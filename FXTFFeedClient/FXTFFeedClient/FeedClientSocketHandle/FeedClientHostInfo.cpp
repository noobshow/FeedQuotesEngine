#include "FeedClientHostInfo.h"
//#include "myLog.h"
#include "FeedClientException.h"

FeedClientHostInfo::FeedClientHostInfo()
{

#ifdef UNIX
	openHostDb();
	// winLog<<"UNIX version FeedClientHostInfo() is called...\n";
#endif
	...
	try 
	{
		...
		if (hostPtr == NULL)
		{
			...
			FeedClientException gethostbynameException(errorCode,errorMsg);
...
FeedClientHostInfo::FeedClientHostInfo(string& hostName,hostTypeClient type)
{
	#ifdef UNIX
		searchHostDB = 0;
	#endif
	...
	try 
	{
		if (type == NAME)
		{
			...
			if (hostPtr == NULL)
			{
				...
				#ifdef UNIX
					FeedClientException gethostbynameException(0,"unix: error getting host by name");
					throw gethostbynameException;
				#endif
			}
        }
		else if (type == ADDRESS)
		{
			...
			if (netAddr == -1)
			{
				FeedClientException inet_addrException(0,"Error calling inet_addr()");
			...
			if (hostPtr == NULL)
			{
					...
					FeedClientException gethostbyaddrException(errorCode,errorMsg);
		...
		{
			FeedClientException unknownTypeException(0,"unknown host type: host name/address has to be given ");
...
#ifdef WINDOWS_XP
void FeedClientHostInfo::detectErrorGethostbyname(int* errCode,string& errorMsg)
{
...
#ifdef WINDOWS_XP
void FeedClientHostInfo::detectErrorGethostbyaddr(int* errCode,string& errorMsg)
{
	*errCode = WSAGetLastError();
...
#ifdef UNIX
char FeedClientHostInfo::getNextHost()
{
