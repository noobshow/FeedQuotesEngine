#include "FeedClientSocket.h"
#include "FeedClientException.h"
//#include "myLog.h"

FeedClientSocket::FeedClientSocket(int pNumber)
{
    portNumber = pNumber;
    blocking = 1;
    ...
    try
    {
		if ( (socketId=socket(AF_INET,SOCK_STREAM,0)) == -1)
        {
				...
				FeedClientException openWinSocketException(errorCode,errorMsg);
				throw openWinSocketException;
			#endif

			#ifdef UNIX
				FeedClientException openUnixSocketException(0,"unix: error getting host by name");
				throw openUnixSocketException;
			#endif
        }
	}
    catch(FeedClientException& excp)
	{
		excp.response();
		//exit(1);
	}
...
void FeedClientSocket::setDebug(int debugToggle)
{
	try 
	{
		if ( setsockopt(socketId,SOL_SOCKET,SO_DEBUG,(char *)&debugToggle,sizeof(debugToggle)) == -1 )
		{
				...
				FeedClientException socketOptionException(errorCode,errorMsg);
				throw socketOptionException;
			#endif

			#ifdef UNIX
				FeedClientException unixSocketOptionException(0,"unix: error getting host by name");
...
void FeedClientSocket::setReuseAddr(int reuseToggle)
{
	try 
	{
...
void FeedClientSocket::setKeepAlive(int aliveToggle)
{
	try 
	{
...
void FeedClientSocket::setLingerSeconds(int seconds)
{
...
void FeedClientSocket::setLingerOnOff(bool lingerOn)
{
	struct linger lingerOption;
...
void FeedClientSocket::setSendBufSize(int sendBufSize)
{
	try 
	{
...
void FeedClientSocket::setReceiveBufSize(int receiveBufSize)
{
	try 
	{
...
void FeedClientSocket::setSocketBlocking(int blockingToggle)
{
    if (blockingToggle)
    {
...
int FeedClientSocket::getReuseAddr()
{
    int myOption;        
    int myOptionLen = sizeof(myOption);
...
int FeedClientSocket::getKeepAlive()
{
    int myOption;        
    int myOptionLen = sizeof(myOption);
...
int FeedClientSocket::getLingerSeconds()
{
	struct linger lingerOption;
...
bool FeedClientSocket::getLingerOnOff()
{
	struct linger lingerOption;
	int myOptionLen = sizeof(struct linger);
...
int FeedClientSocket::getSendBufSize()
{
    int sendBuf;
...
int FeedClientSocket::getReceiveBufSize()
{
    int rcvBuf;
	...
	try 
	{
		if ( getsockopt(socketId,SOL_SOCKET,SO_RCVBUF,(char *)&rcvBuf,&myOptionLen) == -1 )
		{
			...
			#ifdef UNIX
				FeedClientException unixSocketOptionException = new FeedClientException(0,"unix: error getting host by name");
				throw unixSocketOptionException;
			#endif
        }
	}
...
#ifdef WINDOWS_XP
void FeedClientSocket::detectErrorOpenWinSocket(int* errCode,string& errMsg)
{
...
void FeedClientSocket::detectErrorSetSocketOption(int* errCode,string& errMsg)
{
	*errCode = WSAGetLastError();
...
void FeedClientSocket::detectErrorGetSocketOption(int* errCode,string& errMsg)
{
	*errCode = WSAGetLastError();
...
ostream& operator<<(ostream& io,FeedClientSocket& s)
{
	string flagStr = "";
...
FeedClientTcpSocket::~FeedClientTcpSocket()
{
#ifdef WINDOWS_XP
    // Close the winsock library
	// << endl << "system shut down ...";		
	try 
	{
		if (WSACleanup())
		{
			FeedClientException cleanupException(0,"Error: calling WSACleanup()");
...
void FeedClientTcpSocket::initialize()
{
#ifdef WINDOWS_XP
	WSADATA wsaData;
	...
	try 
	{
		...
		{
			FeedClientException initializationException(0,"Error: calling WSAStartup()");
...
void FeedClientTcpSocket::bindSocket()
{
	try
	{
		if (bind(socketId,(struct sockaddr *)&clientAddr,sizeof(struct sockaddr_in))==-1)
		{
				...
				FeedClientException socketBindException(errorCode,errorMsg);
				throw socketBindException;
			#endif
			#ifdef UNIX
				FeedClientException unixSocketBindException(0,"unix: error calling bind()");
...
void FeedClientTcpSocket::detectErrorBind(int* errCode,string& errMsg)
{
	*errCode = WSAGetLastError();
...
void FeedClientTcpSocket::detectErrorRecv(int* errCode,string& errMsg)
{
	*errCode = WSAGetLastError();
...
void FeedClientTcpSocket::detectErrorConnect(int* errCode,string& errMsg)
{
	*errCode = WSAGetLastError();
...
void FeedClientTcpSocket::detectErrorAccept(int* errCode,string& errMsg)
{
	*errCode = WSAGetLastError();
...
void FeedClientTcpSocket::detectErrorListen(int* errCode,string& errMsg)
{
	*errCode = WSAGetLastError();
...
void FeedClientTcpSocket::detectErrorSend(int* errCode,string& errMsg)
{
	*errCode = WSAGetLastError();
...
void FeedClientTcpSocket::connectToServer(string& serverNameOrAddr,hostTypeClient hType)
{ 
	/* 
	   when this method is called, a client socket has been built already,
	   so we have the socketId and portNumber ready.

       a FeedClientHostInfo instance is created, no matter how the server's name is 
	   given (such as www.yuchen.net) or the server's address is given (such
	   as 169.56.32.35), we can use this FeedClientHostInfo instance to get the 
	   IP address of the server
	*/

	FeedClientHostInfo serverInfo(serverNameOrAddr,hType);
	...
	try 
	{
		if (connect(socketId,(struct sockaddr *)&serverAddress,sizeof(serverAddress)) == -1)
		{
				...
				FeedClientException socketConnectException(errorCode,errorMsg);
				throw socketConnectException;
			#endif

			#ifdef UNIX
				FeedClientException unixSocketConnectException(0,"unix: error calling connect()");
...
FeedClientTcpSocket* FeedClientTcpSocket::acceptClient(string& clientHost)
{
	...
	try 
	{
		if ((newSocket = accept(socketId, (struct sockaddr *)&clientAddress,&clientAddressLen)) == -1)
		{
				...
				FeedClientException socketAcceptException(errorCode,errorMsg);
				throw socketAcceptException;
			#endif

			#ifdef UNIX
				FeedClientException unixSocketAcceptException(0,"unix: error calling accept()");
	...
	FeedClientHostInfo clientInfo(string(sAddress),ADDRESSClient);
	char* hostName = clientInfo.getHostName();
    clientHost += string(hostName);
	
    // Create and return the new FeedClientTcpSocket object
    FeedClientTcpSocket* retSocket = new FeedClientTcpSocket();
	retSocket->setSocketId(newSocket);
    return retSocket;
}

void FeedClientTcpSocket::listenToClient(int totalNumPorts)
{
	try 
	{
		if (listen(socketId,totalNumPorts) == -1)
		{
				...
				FeedClientException socketListenException(errorCode,errorMsg);
				throw socketListenException;
			#endif

			#ifdef UNIX
				FeedClientException unixSocketListenException(0,"unix: error calling listen()");
...
/*
int FeedClientTcpSocket::sendMessage(string& message)
{
	...
	try 
	{
		if (numBytes = send(socketId,sendMsg.c_str(),sendMsg.size(),0) == -1)
		{
				...
				FeedClientException socketSendException = new FeedClientException(errorCode,errorMsg);
				throw socketSendException;
			#endif

			#ifdef UNIX
				FeedClientException unixSocketSendException = new FeedClientException(0,"unix: error calling send()");
	...
	*/
	...
	try 
	{
		if (numBytes = send(socketId,sendMsg.c_str(),sendMsg.size(),0) == -1)
		{
				...
				string errorMsg = "error calling send():\n";
				detectErrorSend(&errorCode,errorMsg);
				FeedClientException socketSendException(errorCode,errorMsg);
				throw socketSendException;
			#endif

			#ifdef UNIX
				FeedClientException unixSocketSendException(0,"unix: error calling send()");
				throw unixSocketSendException;
			#endif
        }
	}
...
#ifdef WINDOWS_XP
/*
int FeedClientTcpSocket::XPrecieveMessage(string& message)
{
	...
	try
	{
		while ( numBytes < currentSize )
		{
			...
			if (numBytes == -1)
			{
				...
				FeedClientException socketRecvException = new FeedClientException(errorCode,errorMsg);
	...
	cout   << "[RECV:message length] " << msgLength << endl;
	 << "[RECV:message length] " << msgLength << endl;
	...
	try
	{
		while ( received < msgSize )
		{
			...
			if (numBytes == -1)
			{
				...
				string errorMsg = "error calling recv():\n";
				detectErrorRecv(&errorCode,errorMsg);
				FeedClientException socketRecvException(errorCode,errorMsg);
				throw socketRecvException;
			}
...
int FeedClientTcpSocket::recieveMessage(string& message)
{
	...
	try
	{
        ...
        if (numBytes == -1)
        {
			FeedClientException unixSocketRecvException(0,"unix: error calling recv()");
