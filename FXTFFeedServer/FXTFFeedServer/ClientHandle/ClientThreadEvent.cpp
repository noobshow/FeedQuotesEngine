#include "../stdafx.h"
#include "ClientThreadEvent.h"
#include "../Common/ThreadException.h"


ClientThreadEvent::ClientThreadEvent(string eventName,PAPCFUNC _pfnAPC,BOOL _queueUserAPC)
{
    ...
    mSecurityAttribute->bInheritHandle= TRUE;

	// create the event, it is manual reset, initial state is nonsignaled

	if ((mEvent= CreateEvent(mSecurityAttribute,TRUE,FALSE,eventName.c_str())) == NULL)
	{
		int errCode = 0;
    ...
    if ((pfnAPC=_pfnAPC) != NULL)
    {
		...
		if ((mThread = CreateThread(mSecurityAttribute,0,ThreadProc,this,0, &mThreadID)) == NULL)
		{
			int errCode = 0;
...
ClientThreadEvent::~ClientThreadEvent()
{
...
BOOL ClientThreadEvent::waitForEvent(DWORD waitTime)
{
    DWORD retVal;
	
	if ((retVal = WaitForSingleObjectEx(mEvent,waitTime,TRUE)) == 0xFFFFFFFF)
	{
		int errCode = 0;
...
void ClientThreadEvent::pulseEvent()
{
...
void ClientThreadEvent::setEvent()
{
	if (!SetEvent(mEvent))
	{
		int errCode = 0;
...
void ClientThreadEvent::resetEvent()
{
...
DWORD WINAPI ClientThreadEvent::ThreadProc(LPVOID lpData)
{ 
    // winLog << "[THREAD] the thread is started by OS (ThreadProc) ... " << endl;

    ClientThreadEvent* pEvent = (ClientThreadEvent*)lpData;
	DWORD waitRetVal = 0;
...
void ClientThreadEvent::getErrorDetail(int& errCode,string& header)
{
	char buf[256];
