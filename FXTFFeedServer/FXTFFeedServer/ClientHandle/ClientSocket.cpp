...
ClientSocket::ClientSocket(int pNumber)
{
    portNumber = pNumber;
    blocking = 1;
...
void ClientSocket::setDebug(int debugToggle)
{
	try 
	{
...
void ClientSocket::setReuseAddr(int reuseToggle)
{
	try 
	{
...
void ClientSocket::setKeepAlive(int aliveToggle)
{
	try 
	{
...
void ClientSocket::setLingerSeconds(int seconds)
{
...
void ClientSocket::setLingerOnOff(bool lingerOn)
{
	struct linger lingerOption;
...
void ClientSocket::setSendBufSize(int sendBufSize)
{
	try 
	{
...
void ClientSocket::setReceiveBufSize(int receiveBufSize)
{
	try 
	{
...
void ClientSocket::setSocketBlocking(int blockingToggle)
{
    if (blockingToggle)
    {
...
int ClientSocket::getLingerSeconds()
{
	struct linger lingerOption;
...
bool ClientSocket::getLingerOnOff()
{
	struct linger lingerOption;
	int myOptionLen = sizeof(struct linger);
...
int ClientSocket::getSendBufSize()
{
    int sendBuf;
...
int ClientSocket::getReceiveBufSize()
{
    int rcvBuf;
...
#ifdef WINDOWS_XP
void ClientSocket::detectErrorOpenWinSocket(int* errCode,string& errMsg)
{
...
void ClientSocket::detectErrorSetSocketOption(int* errCode,string& errMsg)
{
	*errCode = WSAGetLastError();
...
void ClientSocket::detectErrorGetSocketOption(int* errCode,string& errMsg)
{
	*errCode = WSAGetLastError();
...
ostream& operator<<(ostream& io,ClientSocket& s)
{
	string flagStr = "";
...
void ClientTcpSocket::initialize()
{
#ifdef WINDOWS_XP
	WSADATA wsaData;
...
void ClientTcpSocket::bindSocket()
{
	try
	{
...
void ClientTcpSocket::detectErrorBind(int* errCode,string& errMsg)
{
	*errCode = WSAGetLastError();
...
void ClientTcpSocket::detectErrorRecv(int* errCode,string& errMsg)
{
	*errCode = WSAGetLastError();
...
void ClientTcpSocket::detectErrorConnect(int* errCode,string& errMsg)
{
	*errCode = WSAGetLastError();
...
void ClientTcpSocket::detectErrorAccept(int* errCode,string& errMsg)
{
	*errCode = WSAGetLastError();
...
void ClientTcpSocket::detectErrorListen(int* errCode,string& errMsg)
{
	*errCode = WSAGetLastError();
...
void ClientTcpSocket::detectErrorSend(int* errCode,string& errMsg)
{
	*errCode = WSAGetLastError();
...
void ClientTcpSocket::connectToServer(string& serverNameOrAddr,hostType hType)
{ 
	/* 
	...
	*/
	...
	ClientHostInfo serverInfo(serverNameOrAddr,hType);
...
ClientTcpSocket* ClientTcpSocket::acceptClient(string& clientHost)
{
	...
	ClientHostInfo clientInfo(string(sAddress),ADDRESS);
	char* hostName = clientInfo.getHostName();
    clientHost += string(hostName);
	
    // Create and return the new myTcpSocket object
    ClientTcpSocket* retSocket = new ClientTcpSocket();
	retSocket->setSocketId(newSocket);
    return retSocket;
}

void ClientTcpSocket::listenToClient(int totalNumPorts)
{
	try 
	{
...
int ClientTcpSocket::sendMessage(string& message,ClientController *aClientController)
{
	...
	*/
	...
	if(data_avalible>0)
	{
		string lInputMessage="";
		//XPrecieveMessage(lInputMessage);
		ReadData(buffer,(data_avalible>sizeof(buffer)?sizeof(buffer):data_avalible));
		cout   <<" Client : "<< lInputMessage;
		//aClientController->acceptClientMessage(lInputMessage);
	}
	...
	try 
	{
		while (lSize>0)
		{
			if ((numBytes = send(socketId,sendMsg.c_str(),sendMsg.size(),0)) < 1)
			{
				...
				{
					int errorCode = 0;
					string errorMsg = "Client-Error calling send():\n";
...
int ClientTcpSocket::XPrecieveMessage(string& message)
{
	...
	try
	{
		while ( received < msgSize )
		{
			...
			if (numBytes == -1)
			{
				mSocketCommunicationErrorOccured=true;
				int errorCode = 0;
				string errorMsg = "Client-error calling recv():\n";
...
int ClientTcpSocket::recieveMessage(string& message)
{
...
bool ClientTcpSocket::ReadData(char *buf,int len)
  {
   int  count=0,ln;
//---- checks
