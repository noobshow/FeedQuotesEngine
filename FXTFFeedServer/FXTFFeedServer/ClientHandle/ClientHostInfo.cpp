...
ClientHostInfo::ClientHostInfo(string& hostName,hostType type)
{
	#ifdef UNIX
		searchHostDB = 0;
	#endif
...
#ifdef WINDOWS_XP
void ClientHostInfo::detectErrorGethostbyname(int* errCode,string& errorMsg)
{
...
#ifdef WINDOWS_XP
void ClientHostInfo::detectErrorGethostbyaddr(int* errCode,string& errorMsg)
{
	*errCode = WSAGetLastError();
