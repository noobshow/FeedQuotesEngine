...
MT4HostInfo::MT4HostInfo()
{
	...
	try 
	{
		...
		if (hostPtr == NULL)
		{
			...
			ThreadException gethostbynameException(errorCode,errorMsg);
...
MT4HostInfo::MT4HostInfo(string& hostName,hostType type)
{
	#ifdef UNIX
		searchHostDB = 0;
	#endif
	...
	try 
	{
		...
		else if (type == ADDRESS)
		{
			...
			if (netAddr == -1)
			{
				ThreadException inet_addrException(0,"Error calling inet_addr()");
			...
			if (hostPtr == NULL)
			{
					...
					ThreadException gethostbyaddrException(errorCode,errorMsg);
		...
		{
			ThreadException unknownTypeException(0,"unknown host type: host name/address has to be given ");
...
#ifdef WINDOWS_XP
void MT4HostInfo::detectErrorGethostbyname(int* errCode,string& errorMsg)
{
...
#ifdef WINDOWS_XP
void MT4HostInfo::detectErrorGethostbyaddr(int* errCode,string& errorMsg)
{
	*errCode = WSAGetLastError();
