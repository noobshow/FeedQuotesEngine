#include "../stdafx.h"
#include "MT4ThreadEvent.h"
#include "../Common/ThreadException.h"


MT4ThreadEvent::MT4ThreadEvent(string eventName,PAPCFUNC _pfnAPC,BOOL _queueUserAPC)
{
...
BOOL MT4ThreadEvent::waitForEvent(DWORD waitTime)
{
    DWORD retVal;
...
void MT4ThreadEvent::pulseEvent()
{
...
void MT4ThreadEvent::resetEvent()
{
...
DWORD WINAPI MT4ThreadEvent::ThreadProc(LPVOID lpData)
{ 
    // winLog << "[THREAD] the thread is started by OS (ThreadProc) ... " << endl;

    MT4ThreadEvent* pEvent = (MT4ThreadEvent*)lpData;
	DWORD waitRetVal = 0;
...
void MT4ThreadEvent::getErrorDetail(int& errCode,string& header)
{
	char buf[256];
