...
const enum SocketLog::logLevels DEFAULT_LOG_LEVEL = SocketLog::LEVEL_1;
...
SocketLog::SocketLog(const string& fileName)
{
    initVars();
    init(fileName);
}

SocketLog::SocketLog(const string& fileName,enum logLevels levelIn)
{
    initVars();
    logLevel = levelIn;
    init(fileName);
...
void SocketLog::init(const string& fileName)
{
   if ( (fileName.c_str())[0] )
...
void SocketLog::init(const string& fileName, int mode)
{
   if ( (fileName.c_str())[0] )
...
void SocketLog::openLog(const string& fileName, int mode)
{
   if (logLevel < QUIET_MODE)
   {
...
void SocketLog::initVars()
{
   time(&startTime);
   logLevel = DEFAULT_LOG_LEVEL;
}

void SocketLog::printHeader(int theEnd)
{
   if ( logLevel < QUIET_MODE )
   {
      clear(ios::goodbit);
...
void SocketLog::getExecTime(int* min, int* sec)
{
   time_t endTime;
   time(&endTime);
