...
FXCMApplicationMessaging::FXCMApplicationMessaging(FXCMFixController* aFXCMFixController)
{
	mFXCMFixController=aFXCMFixController;
	mRequestID=1;
	mFIXLoginStatus=false;
}

FXCMApplicationMessaging::~FXCMApplicationMessaging()
{
}

void FXCMApplicationMessaging::onCreate(const SessionID& aSession_ID)
{
	cout << "Session -> created : " <<aSession_ID<< endl;
	mSessionID = aSession_ID;
}

void FXCMApplicationMessaging::onLogon(const SessionID& aSession_ID)
{
...
void FXCMApplicationMessaging::onLogout(const SessionID& aSession_ID)
{
	mFIXLoginStatus=false;
	...
	mFXCMFixController->fixLogOutOccured(aSession_ID);
}

void FXCMApplicationMessaging::reLoginAfterLogout(const SessionID& aSession_ID)
{
	Session::lookupSession(aSession_ID)->logon();
}

void FXCMApplicationMessaging::toAdmin(Message& aToAdminMessage, const SessionID& aSession_ID)
{
...
void FXCMApplicationMessaging::toApp(Message& aToAppMessage, const SessionID& aSession_ID)
{
	string lTargetSubID = mSessionSettings->get().getString("TargetSubID");		//No Need For OCBC May need to Add SenderCompID
	aToAppMessage.getHeader().setField(TargetSubID(lTargetSubID));				//No Need For OCBC May need to Add SenderCompID
}

void FXCMApplicationMessaging::fromAdmin(const Message& aFromAdminMessage, const SessionID& aSession_ID)
{
	cout << "From Admin  Message : "<<aFromAdminMessage<<endl;
	crack(aFromAdminMessage, aSession_ID);
}

void FXCMApplicationMessaging::fromApp(const Message& aFromAppMessage, const SessionID& aSession_ID)
{
	crack(aFromAppMessage, aSession_ID);
}

void FXCMApplicationMessaging::onMessage(const FIX44::TradingSessionStatus& aTradingSessionStatus, const SessionID& aSession_ID)
{
	...
	cout << "[FXCM]TradingSessionStatus -> TradSesStatus -" << trad_status << endl;
	/*
	...
	*/
	mFXCMFixController->StartGettingMarketData(atoi(trad_status.c_str()));
}

void FXCMApplicationMessaging::onMessage(const FIX44::MarketDataRequestReject& aMarketDataRequestReject, const SessionID& aSession_ID)
{
	cout << "[FXCM]MarketDataRequestReject -> " <<aMarketDataRequestReject<< endl;
...
void FXCMApplicationMessaging::onMessage(const FIX44::MarketDataSnapshotFullRefresh& aMarketDataSnapshotFullRefresh, const SessionID& aSession_ID)
{
	...
	lSymbolSourceModel->setFeedProviderID(mFXCMFixController->getFeedProviderID());
	...
	{
		mFXCMFixController->UpdatedMarketData(lSymbolSourceModel);
	}
}

void FXCMApplicationMessaging::StartFixSession()
{
	try{
		mSessionSettings		= new SessionSettings("settings_fxcm.cfg");
...
void FXCMApplicationMessaging::EndFixSession()
{
...
void FXCMApplicationMessaging::GetTradingSessionStatus()
{
...
void FXCMApplicationMessaging::SubscribeForMarketData(list<SymbolValues*> aAvailableSymbols)
{
...
void FXCMApplicationMessaging::UnsubscribeForMarketData()
{
...
string FXCMApplicationMessaging::GetNextRequestID()
{
	if(mRequestID == 65535)
		mRequestID = 1;
